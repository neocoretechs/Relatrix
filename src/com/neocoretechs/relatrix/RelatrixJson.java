...
public final class RelatrixJson {
	...
	*/
	 ...
	 * @throws IOException problem getting BufferedMap
	 * @throws IllegalAccessException problem creating morphic class
	 */
	public static Comparable<?>[] getTuple(Object d, Object m, Object r)  throws IOException, IllegalAccessException {
		JSONObject jsono;
	...
	/**
	 * Get a tuple for the triplet specified, translating JSONObjects to morphic classes if necessary
	 * @param alias alias for database
	 * @param d domain
	 * @param m map
	 * @param r range
	 * @return translated tuple
	 ...
	 * @throws IllegalAccessException problem creating morphic class
	 */
	public static Comparable<?>[] getTuple(Alias alias, Object d, Object m, Object r)  throws IOException, IllegalAccessException {
		JSONObject jsono;
		Comparable<?> jkeyd, jkeym, jkeyr;
		if(d instanceof JSONObject) {
			jsono = (JSONObject)d;
	...
	public static Relation store(Object d, Object m, Object r) throws IllegalAccessException, IOException, DuplicateKeyException, ClassNotFoundException {
		...
		Comparable<?>[] tuple = getTuple(d,m,r);
	...
	public static List store(ArrayList<Comparable[]> tuples) throws IOException, IllegalAccessException, ClassNotFoundException {
		...
		identity.setDomainKey(pk.getDomainKey());
		identity.setMapKey(pk.getMapKey());
		DBKey rKey = AbstractRelation.checkMorphism(translatedTuple[2]);
		if(rKey == null)
			identity.setRange(translatedTuple[2]);
		else {
			identity.setRangeKey(rKey);
			identity.setRangeResolved(translatedTuple[2]);
		}
		if(pk.getIdentity() == null) {
			identity.setDomainResolved(translatedTuple[0]);
			identity.setMapResolved(translatedTuple[1]);
			// newKey will call into DBKey.newKey with proper transactionId and alias
			// and then call proper indexInstanceTable.put(instance) to place the DBKey/instance instance/DBKey
			// and return the new DBKey reference
			identity.setIdentity(identity.newKey(identity));
				...
				public void run() {
					if(semaphore.get() == 0) // error flag, if any task increments, abort write
						try {
							...
							if( DEBUG  )
								System.out.println("RelatrixJson.store stored :"+dmr);
						} catch (IllegalAccessException | IOException | DuplicateKeyException e) {
							if(semaphore.get() == 0)
								writeException.initCause(e);
							else
	...
	private static void removeSearch(DBKey c, List<DBKey> deleted, ParallelExecutionContext ctx) throws IOException, IllegalArgumentException, ClassNotFoundException, IllegalAccessException, NoSuchElementException, DuplicateKeyException {
		...
		Iterator<?> itd = new RelatrixIteratorJson(dmr, dmr_return, ctx); //findSet(c,"*","*");
		Iterator<?> itm = new RelatrixIteratorJson(mdr, mdr_return, ctx); //findSet("*",c,"*");
		Iterator<?> itr = new RelatrixIteratorJson(rmd, rmd_return, ctx); //findSet("*","*",c);
	...
	private static void removeSearch(Alias alias, DBKey c, List<DBKey> deleted, ParallelExecutionContext ctx) throws IOException, IllegalArgumentException, ClassNotFoundException, IllegalAccessException, NoSuchElementException, DuplicateKeyException {
		...
		short mdr_return[] = new short[]{-1,2,0,2};
		short rmd_return[] = new short[]{-1,2,2,0};
		Iterator<?> itd = new RelatrixIteratorJson(alias, dmr, dmr_return, ctx); //findSet(alias, transactionId, c,"*","*");
		Iterator<?> itm = new RelatrixIteratorJson(alias, mdr, mdr_return, ctx); //findSet(alias, transactionId, "*",c,"*");
		Iterator<?> itr = new RelatrixIteratorJson(alias, rmd, rmd_return, ctx); //findSet(alias, transactionId, "*","*",c);
		sequentialSearch(itd, itm, itr, deleted);
	}
	...
	private static void relatedSearch(Alias alias, DBKey c, List<DBKey> dbkeys, ParallelExecutionContext ctx) throws IOException, IllegalArgumentException, ClassNotFoundException, IllegalAccessException, NoSuchElementException {
		...
		short mdr_return[] = new short[]{-1,2,0,2};
		short rmd_return[] = new short[]{-1,2,2,0};
		Iterator<?> itd = new RelatrixIteratorJson(alias, dmr, dmr_return, ctx); //findSet(c,"*","*");
		Iterator<?> itm = new RelatrixIteratorJson(alias, mdr, mdr_return, ctx); //findSet("*",c,"*");
		Iterator<?> itr = new RelatrixIteratorJson(alias, rmd, rmd_return, ctx); //findSet("*","*",c);
		sequentialMorphismSearch(itd, dbkeys);
		sequentialMorphismSearch(itm, dbkeys);
		sequentialMorphismSearch(itr, dbkeys);
	}
	 ...
	 * In the wildcard specification: findSet("*","*","*"), which will return all elements of the
	 * domain->map->range relationships, or the case of findSet(object,object,object), which return one element matching the
	 ...
	 */
	@ServerMethod
	public static List<? extends Comparable> findSetParallel(List<Object> d, Character m, Character r) {
		...
		for(int i = 0; i < d.size(); i++) {
			...
			futures.add( SynchronizedThreadManager.getInstance().submitWithContext(new Callable<Object>() {
				...
				public List<Result> call() {
					...
					try {
						findStream(d.get(taskId), m, r).forEach(e->{
							res.add((Result)e);
						});
					} catch (IllegalArgumentException | ClassNotFoundException | IllegalAccessException | IOException e) {
						throw new RuntimeException(e);
	 ...
	 */
	@ServerMethod
	public static List<? extends Comparable> findSetParallel(Character d, List<Object> m, Character r) {
		List<Future<Object>> futures = new ArrayList<>();
		for(int i = 0; i < m.size(); i++) {
			...
			futures.add( SynchronizedThreadManager.getInstance().submitWithContext(new Callable<Object>() {
				...
				public List<Result> call() {
					List<Result> res = new ArrayList<Result>();
					try {
						findStream(d, m.get(taskId), r).forEach(e-> {
							res.add((Result)e);
						});
					} catch (IllegalArgumentException | ClassNotFoundException | IllegalAccessException | IOException e) {
						throw new RuntimeException(e);
	 ...
	 */
	@ServerMethod
	public static List<? extends Comparable> findSetParallel(Character d, Character m, List<Object> r) {
		List<Future<Object>> futures = new ArrayList<>();
		for(int i = 0; i < r.size(); i++) {
			...
			futures.add( SynchronizedThreadManager.getInstance().submitWithContext(new Callable<Object>() {
				...
				public List<Result> call() {
					List<Result> res = new ArrayList<Result>();
					try {
						findStream(d, m, r.get(taskId)).forEach(e-> {
							res.add((Result)e);
						});
					} catch (IllegalArgumentException | ClassNotFoundException | IllegalAccessException | IOException e) {
						throw new RuntimeException(e);
	 ...
	 */
	@ServerMethod
	public static List<? extends Comparable> findSetParallel(Alias alias, List<Object> d, Character m, Character r) {
		List<Future<Object>> futures = new ArrayList<>();
		for(int i = 0; i < d.size(); i++) {
			final int taskId = i;
			futures.add( SynchronizedThreadManager.getInstance().submitWithContext(new Callable<Object>() {
				...
				public List<Result> call() {
					List<Result> res = new ArrayList<Result>();
					try {
						findStream(alias, d.get(taskId), m, r).forEach(e->{
							res.add((Result)e);
						});
					} catch (IllegalArgumentException | ClassNotFoundException | IllegalAccessException | IOException e) {
						throw new RuntimeException(e);
	 ...
	 */
	@ServerMethod
	public static List<? extends Comparable> findSetParallel(Alias alias, Character d, List<Object> m, Character r) {
		List<Future<Object>> futures = new ArrayList<>();
		for(int i = 0; i < m.size(); i++) {
			final int taskId = i;
			futures.add( SynchronizedThreadManager.getInstance().submitWithContext(new Callable<Object>() {
				...
				public List<Result> call() {
					List<Result> res = new ArrayList<Result>();
					try {
						findStream(alias, d, m.get(taskId), r).forEach(e->{
							res.add((Result)e);
						});
					} catch (IllegalArgumentException | ClassNotFoundException | IllegalAccessException | IOException e) {
						throw new RuntimeException(e);
	 ...
	 */
	@ServerMethod
	public static List<? extends Comparable> findSetParallel(Alias alias, Character d, Character m, List<Object> r) {
		List<Future<Object>> futures = new ArrayList<>();
		for(int i = 0; i < r.size(); i++) {
			final int taskId = i;
			futures.add( SynchronizedThreadManager.getInstance().submitWithContext(new Callable<Object>() {
				...
				public List<Result> call() {
					List<Result> res = new ArrayList<Result>();
					try {
						findStream(alias, d, m, r.get(taskId)).forEach(e-> {
							res.add((Result)e);
						});
					} catch (IllegalArgumentException | ClassNotFoundException | IllegalAccessException | IOException e) {
						throw new RuntimeException(e);
	 ...
	 * work against, so in this method that check is performed in the createHeadsetFactory method. If you are going to anchor a set
	 * retrieval and declare it a 'head' or 'tail' relative to an object, you need a concrete object to assert that retrieval.
	 * @param darg Domain of morphism, a dont-care wildcard "*", a return-                                                                                                                                           
	 */
	@ServerMethod
