...
public final class RelatrixJsonTransaction {
	...
	private RelatrixJsonTransaction() {
	}
	// 2.) volatile instance
	private static volatile RelatrixJsonTransaction instance = null;
	// 3.) lock class, assign instance if null
	public static RelatrixJsonTransaction getInstance() {
		synchronized(RelatrixJsonTransaction.class) {
			if(instance == null) {
				...
				try {
					...
					RelatrixKVJsonTransaction.classLoader.connectToLocalRepository(true, true); // transaction param true, json true
	...
	*/
			...
			try {
				...
				if(tuplep[0] instanceof JSONObject) {
					...
					if(tuplep[0] instanceof Comparable<?>) {
						jkeyd = (Comparable<?>)tuplep[0];
					} else {
						throw new IllegalAccessException("Domain type must be JSONObject or Comparable for:"+tuplep[0]+" found:"+tuplep[0].getClass());
					}
				}
				if(tuplep[1] instanceof JSONObject) {
					...
					if(tuplep[1] instanceof Comparable<?>) {
						jkeym = (Comparable<?>)tuplep[1];
					} else {
						throw new IllegalAccessException("Map type must be JSONObject or Comparable for:"+tuplep[1]+" found:"+tuplep[1].getClass());
					}
				}
	...
	/**
	 * Store a primary key and its related indexes in parallel threads
	 * @param xid transaction id
	...
	/**
	 * Store a Relation, primary key and its indexes in parallel threads
	 * @param alias The database alias
	 * @param xid Transaction Id
	 * @param identity Relation payload
	 * @param pk Primary key of Relation
	...
	public static Stream<?> findStream(TransactionId xid, Object darg, Object marg, Object rarg) throws IOException, IllegalArgumentException, ClassNotFoundException, IllegalAccessException {
		IteratorFactory ifact = new FindSetMode7JsonTransaction(xid, darg, marg, rarg);
		return new RelatrixStream(ifact.createIterator(new ParallelExecutionContext(new IndexResolver(true),null)));
	}
	@ServerMethod
	...
	public static Stream<?> findStream(Alias alias, TransactionId xid, Object darg, Object marg, Object rarg) throws IOException, IllegalArgumentException, ClassNotFoundException, IllegalAccessException {
		IteratorFactory ifact = new FindSetMode7JsonTransaction(xid, darg, marg, rarg);
		return new RelatrixStream(ifact.createIterator(alias,new ParallelExecutionContext(new IndexResolver(true),null)));
	}
	@ServerMethod
	...
	/**
	 * Find a subset of relationships based on object instances or class declarations
	 * @param darg
	 * @param marg
	 * @param rarg
	 * @return
