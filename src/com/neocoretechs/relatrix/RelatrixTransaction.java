...
public final class RelatrixTransaction {
	...
	static {
		...
		sftpm.init(new String[] {storeXTransaction});
		sftpm.init(new String[] {deleteXTransaction});
		sftpm.init(new String[] {storeITransaction});
		sftpm.init(new String[] {searchXTransaction});
	...
	*/
	...
	public static Relation store(TransactionId xid, Comparable<?> d, Comparable<?> m, Comparable<?> r) throws IllegalAccessException, IOException, DuplicateKeyException, ClassNotFoundException {
		...
		PrimaryKeySet pk = PrimaryKeySet.locate(xid, d, m, ctx);
	...
	public static Relation store(Alias alias, TransactionId xid, Comparable<?> d, Comparable<?> m, Comparable<?> r) throws IllegalAccessException, IOException, DuplicateKeyException, ClassNotFoundException {
		...
		// If the search winds up at the key or the key is empty or the domain->map exists, the key
		// cannot be inserted
		PrimaryKeySet pk = PrimaryKeySet.locate(alias, xid, d, m, ctx);
		if(pk.getIdentity() == null) {
			identity.setDomainKey(pk.getDomainKey());
			identity.setMapKey(pk.getMapKey());
			identity.setDomainResolved(d);
	...
	public static List store(TransactionId xid, ArrayList<Comparable[]> tuples) throws IOException, IllegalAccessException, ClassNotFoundException {
		...
		ParallelExecutionContext ctx = new ParallelExecutionContext(new IndexResolver(),null);
		identity.setTransactionId(xid);
		PrimaryKeySet pk = PrimaryKeySet.locate(xid, tuple[0], tuple[1], ctx);
		identity.setDomainKey(pk.getDomainKey());
		identity.setMapKey(pk.getMapKey());
	...
	public static List store(Alias alias, TransactionId xid, ArrayList<Comparable[]> tuples) throws IOException, IllegalAccessException, ClassNotFoundException {
		...
		identity.setAlias(alias);
		identity.setTransactionId(xid);
		PrimaryKeySet pk = PrimaryKeySet.locate(alias, xid, tuple[0], tuple[1], ctx);
		identity.setDomainKey(pk.getDomainKey());
		identity.setMapKey(pk.getMapKey());
		DBKey rKey = AbstractRelation.checkMorphism(tuple[2]);
		if(rKey == null)
	...
	/**
	 * Private helper to facilitate bulk removal
	 * @param transactionId
	 * @param c
	 * @param deleted
	...
	private static void removeSearch(TransactionId transactionId, DBKey c, List<DBKey> deleted, ParallelExecutionContext ctx) throws IOException, IllegalArgumentException, ClassNotFoundException, IllegalAccessException, NoSuchElementException, DuplicateKeyException {
		...
		Iterator<?> itd = new RelatrixIteratorTransaction(transactionId, dmr, dmr_return, ctx); //findSet(transactionId, c,'*','*');
		Iterator<?> itm = new RelatrixIteratorTransaction(transactionId, mdr, mdr_return, ctx); //findSet(transactionId, '*',c,'*');
		Iterator<?> itr = new RelatrixIteratorTransaction(transactionId, rmd, rmd_return, ctx); //findSet(transactionId, '*','*',c);
	...
	private static void removeSearch(Alias alias, TransactionId transactionId, DBKey c, List<DBKey> deleted, ParallelExecutionContext ctx) throws IOException, IllegalArgumentException, ClassNotFoundException, IllegalAccessException, NoSuchElementException, DuplicateKeyException {
		...
		short mdr_return[] = new short[]{-1,2,0,2};
		short rmd_return[] = new short[]{-1,2,2,0};
		Iterator<?> itd = new RelatrixIteratorTransaction(alias, transactionId, dmr, dmr_return, ctx); //findSet(alias, transactionId, c,'*','*');
		Iterator<?> itm = new RelatrixIteratorTransaction(alias, transactionId, mdr, mdr_return, ctx); //findSet(alias, transactionId, '*',c,'*');
		Iterator<?> itr = new RelatrixIteratorTransaction(alias, transactionId, rmd, rmd_return, ctx); //findSet(alias, transactionId, '*','*',c);
		sequentialSearch(itd, itm, itr, deleted);
	}

	/**
	 * Remove array of keys and all indexes
	 * @param transactionId current xid
	 * @param removed List of DBKey to remove
	 * @throws IllegalArgumentException
	 * @throws ClassNotFoundException
	 * @throws IllegalAccessException
	 * @throws NoSuchElementException
	 ...
	 */
	@ServerMethod
	public static List<? extends Comparable> findSetParallel(TransactionId transactionId, List<Object> d, Character m, Character r) throws IOException {
		if(DEBUG)
			System.out.printf("findSetParallel Domain size:%d%n", d.size());
	 ...
	 */
	@ServerMethod
	public static List<? extends Comparable> findSetParallel(TransactionId transactionId, Character d, List<Object> m, Character r) throws IOException {
		if(DEBUG)
			System.out.printf("findSetParallel Map size:%d%n", m.size());
		List<Future<Object>> futures = new ArrayList<>();
	...
	public static List<? extends Comparable> findSetParallel(Alias alias, TransactionId transactionId, List<Object> d, Character m, Character r) {
		if(DEBUG)
			System.out.printf("findSetParallel Alias %s Domain size:%d%n", alias, d.size());
		List<Future<Object>> futures = new ArrayList<>();
		for(int i = 0; i < d.size(); i++) {
			final int taskId = i;
			futures.add( SynchronizedThreadManager.getInstance().submitWithContext(new Callable<Object>() {
	...
	private static void relatedSearch(TransactionId xid, DBKey c, List<DBKey> dbkeys, ParallelExecutionContext ctx) throws IOException, IllegalArgumentException, ClassNotFoundException, IllegalAccessException, NoSuchElementException {
		...
		short mdr_return[] = new short[]{-1,2,0,2};
		short rmd_return[] = new short[]{-1,2,2,0};
		Iterator<?> itd = new RelatrixIteratorTransaction(xid, dmr, dmr_return, ctx); //findSet(c,"*","*");
		Iterator<?> itm = new RelatrixIteratorTransaction(xid, mdr, mdr_return, ctx); //findSet("*",c,"*");
		Iterator<?> itr = new RelatrixIteratorTransaction(xid, rmd, rmd_return, ctx); //findSet("*","*",c);
	...
	public static List<Comparable> findSet(Alias alias, TransactionId xid, Object c) throws IOException, IllegalArgumentException, ClassNotFoundException, IllegalAccessException {
		...
		if(!(c instanceof AbstractRelation)) {
			if(c instanceof Tuple) {
				if(((Tuple)c).getRelation() != null) {
					if(((Tuple)c).getRelation().getIdentity() == null)
						IndexInstanceTable.resolve(((Tuple)c).getRelation());
					located.add(((Tuple)c).getRelation());
					relatedTupleSearch(alias, xid, ((Tuple)c).getRelation().getIdentity(), dbkeys, ctx);
					...
					if(pk.getIdentity() != null) {
						...
						relatedTupleSearch(alias, xid, pk.getIdentity(), dbkeys, ctx);
						keysToInstances(alias, xid, dbkeys, located);
						return located;
					}
				}
			...
			dbk = ((AbstractRelation)c).getIdentity();
			if(dbk == null)
				IndexInstanceTable.resolve((AbstractRelation) c);
			dbkeys.add(dbk);
		}
		relatedSearch(alias, xid, dbk, dbkeys, ctx);
		int index = 0;
		while(index < dbkeys.size()) {
			relatedSearch(alias, xid, dbkeys.get(index), dbkeys, ctx);
			++index;
		}
		// should have unique list of dbkeys
		keysToInstances(alias, xid, dbkeys, located);
	...
	private static void relatedSearch(Alias alias, TransactionId xid, DBKey c, List<DBKey> dbkeys, ParallelExecutionContext ctx) throws IOException, IllegalArgumentException, ClassNotFoundException, IllegalAccessException, NoSuchElementException {
		...
		short mdr_return[] = new short[]{-1,2,0,2};
		short rmd_return[] = new short[]{-1,2,2,0};
		Iterator<?> itd = new RelatrixIteratorTransaction(alias, xid, dmr, dmr_return, ctx); //findSet(c,"*","*");
		Iterator<?> itm = new RelatrixIteratorTransaction(alias, xid, mdr, mdr_return, ctx); //findSet("*",c,"*");
		Iterator<?> itr = new RelatrixIteratorTransaction(alias, xid, rmd, rmd_return, ctx); //findSet("*","*",c);
		Relatrix.sequentialMorphismSearch(itd, dbkeys);
		Relatrix.sequentialMorphismSearch(itm, dbkeys);
		Relatrix.sequentialMorphismSearch(itr, dbkeys);
	}
	 ...
	 * @param darg Object for domain of relationship, a dont-care wildcard '*', or class template
	 * @param marg Object for the map of relationship , a dont-care wildcard '*', or a class template
	 * @param rarg Object for the range of the relationship, a dont-care wildcard '*', or a class template
	 * @param endarg variable length set of parameters qualifying the non-concrete (wildcard) parameters. Either of Class or instance type.
	 * @exception IOException low-level access or problems modifiying schema
	 * @exception IllegalArgumentException At least one argument must be a valid object reference instead of a wildcard *
	 * @exception ClassNotFoundException if the Class of Object is invalid
	 * @throws IllegalAccessException 
	 ...
	 * @param arg1 Qualifier for domain wildcard or Class or instance greater than result
	 * @param arg2 Qualifier for map wildcard or  Class or instance greater than result
	 ...
	 * @param darg Domain of morphism, a dont-care wildcard '*', or class
	 * @param marg Map of morphism relationship, a dont-care wildcard '*', or class
	 * @param rarg Range or codomain or morphism relationship, a dont-care wildcard '*', or class
	 * @param endarg variable length set of parameters qualifying the non-concrete (wildcard) parameters. Either of Class or instance type.
	 * @return The RelatrixIterator from which the data may be retrieved. Follows Iterator interface, return Iterator<Result>
	 * @throws IOException
	 * @throws IllegalArgumentException At least one argument must be a valid object reference instead of a wildcard *
	 * @throws ClassNotFoundException
	 * @throws IllegalAccessException
	 */
	@ServerMethod
	...
	public static void loadClassFromPath(String pack, String path) throws IOException {
		...
		RelatrixKVTransaction.classLoader.setBytesInRepository(pack,p);
	}
	/**
	...
	public static void loadClassFromJar(String jar) throws IOException {
		RelatrixKVTransaction.classLoader.setBytesInRepositoryFromJar(jar);
	}
	@ServerMethod
