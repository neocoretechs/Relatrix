 ...
 * The process method calls setServerObjectReturn with the result of the invoked method, and in the case of an Iterator,
...
public class RelatrixTransactionStatement extends RelatrixStatement implements RelatrixTransactionStatementInterface, Serializable {
    ...
    public RelatrixTransactionStatement(UUID session, String tmeth, Object ... o1) {
	...
	public RelatrixTransactionStatement(TransactionId transactionId, UUID session) {
		super(session);
		this.transactionId = transactionId;
	}
    
    @Override
    public synchronized String toString() { return String.format("%s for Session:%s xid:%s Method:%s param len:%d return Class:%s%n",
             this.getClass().getName(), getSession(), transactionId, methodName,
             (paramArray == null ? 0 : paramArray.length), returnClass); }
