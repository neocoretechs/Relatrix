 ...
 *
 */
public class RelatrixJsonKVStatement implements Serializable, RelatrixStatementInterface {
    ...
    public RelatrixJsonKVStatement(String session) {
     ...
     */
    public RelatrixJsonKVStatement(String tmeth, Object ... o1) {
	...
	public synchronized void process() throws Exception {
		...
		Object result = RelatrixKVJsonServer.relatrixMethods.invokeMethod(this);
		...
		if( result != null && !((result instanceof Serializable) && !(result instanceof Externalizable))) {					
			...
			RemoteIteratorKVJsonClient ric = null;
			if(result.getClass() == IteratorWrapper.class) {	
				ric = new RemoteIteratorKVJsonClient(((InetSocketAddress)RelatrixKVJsonServer.address).getAddress().getHostName(), 
							RelatrixKVJsonServer.findIteratorServerPort("com.neocoretechs.relatrix.iterator.IteratorWrapper"));
			} else {
				throw new Exception("Processing chain not set up to handle intermediary for non serializable object "+result);
			}
			// Link the object instance to session for later method invocation
			RelatrixKVJsonServer.sessionToObject.put(ric.getSession(), result);
