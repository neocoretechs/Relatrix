...
public class RemoteIteratorKVJsonClient implements Runnable, RelatrixStatementInterface, Serializable, Iterator {
	...
	public void run() {
		...
		try {
			while(shouldRun) {
				...
				if(DEBUG)
					System.out.println("RemoteIteratorKVJsonClient read "+inJson+" from "+workerSocket);
	...
	public String toString() {
		return String.format("%s RemoteNode:%s RemotePort:%d workerSocket out socket:%s, session:%s method:%s return:%s%n",this.getClass().getName(), remoteNode, remotePort, workerSocket, session, methodName, objectReturn);
	}
