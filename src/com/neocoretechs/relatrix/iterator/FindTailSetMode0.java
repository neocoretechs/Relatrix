...
public class FindTailSetMode0 extends FindSetMode0 {
	...
	protected Iterator<?> createRelatrixIterator(Morphism tdmr) throws IllegalAccessException, IOException {
		...
		if(tdmr.getMap() == null) {
			if(endarg[1] instanceof Class) {
				xdmr.setMap((Comparable) Relatrix.firstKey((Class)endarg[1]));
			} else {
		...
		if(tdmr.getRange() == null) {
			if(endarg[2] instanceof Class) {
				xdmr.setRange((Comparable) Relatrix.firstKey((Class)endarg[2]));
			} else {
	...
	protected Iterator<?> createRelatrixIterator(Alias alias, Morphism tdmr) throws IllegalAccessException, IOException, NoSuchElementException {
		...
		if(tdmr.getMap() == null) {
			if(endarg[1] instanceof Class) {
				xdmr.setMap(alias,(Comparable) Relatrix.firstKey(alias,(Class)endarg[1]));
			} else {
		...
		if(tdmr.getRange() == null) {
			if(endarg[2] instanceof Class) {
				xdmr.setRange(alias,(Comparable) Relatrix.firstKey(alias,(Class)endarg[2]));
			} else {
