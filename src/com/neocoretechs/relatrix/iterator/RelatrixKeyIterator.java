 ...
 *
 */
public class RelatrixKeyIterator implements Iterator<Comparable> {
     ...
     * @throws IOException 
     */
    public RelatrixKeyIterator(TransactionalTreeMap bts, Comparable template) throws IOException {
    	this.deepStore = bts;
    	iter = (TailSetIterator) bts.tailMap(template);
    	if( iter.hasNext() ) {
	...
	public boolean hasNext() {
		if( DEBUG )
			System.out.println("RelatrixKeyIterator.hasNext() "+iter.hasNext()+" "+needsIter+" "+buffer+" "+nextit);
		return needsIter;
	}
	...
	public Comparable next() {
		if( buffer == null || needsIter) {
			if( DEBUG ) {
	    			System.out.println("RelatrixKeyIterator.next() before iteration:"+iter.hasNext()+" "+needsIter+" "+buffer+" "+nextit);
			}
			if( nextit != null )
				buffer = nextit;
		...
		if( DEBUG ) {
			System.out.println("RelatrixKeyIterator.next() template match after iteration:"+iter.hasNext()+" "+needsIter+" "+buffer+" "+nextit);
		}
		return nextit;
	}
