...
public class RelatrixSubsetIterator implements Iterator<Result> {
	...
	protected Morphism template, templatez;
     ...
     * @throws IOException
     */
    public RelatrixSubsetIterator(Morphism template, Morphism templatez, Morphism templateo, Morphism templatep, short[] dmr_return) throws IOException {
      	if(DEBUG)
    	...
    	try {
    		...
    		if(dmr_return[1] != 0) {
    			iter1 = RelatrixKV.findSubMapKV(template.getDomain(), templatez.getDomain());
    			needsIter1 = true;
    		} else {
    			buffer.setDomain(template.getDomain());
    		}
    		if(dmr_return[2] != 0) {
    			iter2 = RelatrixKV.findSubMapKV(template.getMap(), templatez.getDomain());
    			needsIter2 = true;
    		} else {
     ...
     */
    public RelatrixSubsetIterator(String alias, Morphism template, Morphism templatez, Morphism templateo, Morphism templatep, short[] dmr_return) throws IOException, NoSuchElementException {
     	if(DEBUG)
    		System.out.printf("%s %s %s%n", this.getClass().getName(), template, Arrays.toString(dmr_return));
     	this.alias = alias;
    	this.template = template;
    	...
    	try {
    		...
    		if(dmr_return[1] != 0) {
    			iter1 = RelatrixKV.findSubMapKV(alias, template.getDomain(), templatez.getDomain());
    			needsIter1 = true;
    		} else {
    			buffer.setDomain(template.getDomain());
    		}
    		if(dmr_return[2] != 0) {
    			iter2 = RelatrixKV.findSubMapKV(alias, template.getMap(), templatez.getDomain());
    			needsIter2 = true;
    		} else {
    			buffer.setMap(template.getMap());
    		}
		...
		while(true) {
			...
			if(needsIter2) {
				if(iter2.hasNext()) {
					...
					needsIter2 = true;
					try {
						iter2 = RelatrixKV.findSubMapKV(template.getMap(), templatez.getMap());
		...
		while(true) {
			...
			if(needsIter2) {
				if(iter2.hasNext()) {
					...
					needsIter2 = true;
					try {
						iter2 = RelatrixKV.findSubMapKV(alias, template.getMap(), templatez.getMap());
					} catch (IllegalArgumentException | ClassNotFoundException | IllegalAccessException | IOException e) {
						throw new RuntimeException(e);
					}
					if(DEBUG)
