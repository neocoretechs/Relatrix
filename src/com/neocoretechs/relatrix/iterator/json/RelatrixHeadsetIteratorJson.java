 ...
 */
public class RelatrixHeadsetIteratorJson implements Iterator<Result> {
    ...
    public RelatrixHeadsetIteratorJson() {}
    /**
     ...
     * @throws IOException 
     */
    public RelatrixHeadsetIteratorJson(AbstractRelation template, AbstractRelation templateo, short[] dmr_return, ParallelExecutionContext ctx) throws IOException {
    	if(DEBUG)
    	...
    	try {
    		if(template.getDomain() != null) {
    			// exact match instance
       			DBKey dk = (DBKey) RelatrixKVJson.get(template.getDomain());
    			if(dk != null) {
    				dkey.add(dk);
    				dkeyLo = dk;
    				dkeyHi = dk;
    			...
    			if(templateo.getDomain() != null) {
    				...
    				*/
    				ConcurrentLinkedQueue<DBKey> q = RelatrixKVJson.findHeadMapKVStream(templateo.getDomain())
    		...
    		if(template.getMap() != null) {
    			...
    			if(templateo.getMap() != null) {
    				...
    				ConcurrentLinkedQueue<DBKey> q = RelatrixKVJson.findHeadMapKVStream(templateo.getMap())
  						.map(e -> ((Map.Entry<Comparable,DBKey>) e).getValue())
			    	.collect(Collectors.toCollection(ConcurrentLinkedQueue::new));
    		...
    		if(template.getRange() != null) {
    			...
    			if(templateo.getRange() != null) {
    				...
    				rkey.add(rkeys);
     				...
     				ConcurrentLinkedQueue<DBKey> q = RelatrixKVJson.findHeadMapKVStream(templateo.getRange())
      						.map(e -> ((Map.Entry<Comparable,DBKey>) e).getValue())
    			    	.collect(Collectors.toCollection(ConcurrentLinkedQueue::new));
 		...
 		FindsetUtilJson.getMorphismRange(dkeyLo, mkeyLo, rkeyLo, dkeyHi, mkeyHi, rkeyHi, dkey, mkey, rkey, resultSet);
		
 		if(DEBUG) {
			System.out.println(">>Result set size:"+resultSet.size());
    		resultSet.values().iterator().forEachRemaining(e->{
    			try {
					System.out.println(">>"+RelatrixKVJson.get(e));
     ...
     * @throws IOException
     */
    public RelatrixHeadsetIteratorJson(Alias alias, AbstractRelation template, AbstractRelation templateo, short[] dmr_return, ParallelExecutionContext ctx) throws IOException {
    	this.alias = alias;
      	if(DEBUG)
    	...
    	try {
    		...
    		if(template.getDomain() != null) {
    			...
    			if(templateo.getDomain() != null) {
    				/*RelatrixKVJson.findHeadMapKVStream(alias,templateo.getDomain()).forEach(e -> {
      				...
      				ConcurrentLinkedQueue<DBKey> q = RelatrixKVJson.findHeadMapKVStream(alias,templateo.getDomain())
      	    			    .map(e -> ((Map.Entry<Comparable,DBKey>) e).getValue())
      	    			    .collect(Collectors.toCollection(ConcurrentLinkedQueue::new));
      	    				dkey.addAll(q); // single-threaded merge
      	    				// compute lo/hi
    		...
    		if(template.getMap() != null) {
    			...
    			if(templateo.getMap() != null) {
    				/*RelatrixKVJson.findHeadMapKVStream(alias,templateo.getMap()).forEach(e -> {
    					DBKey mkeys = ((Map.Entry<Comparable,DBKey>)e).getValue();
    					if(mkeys.compareTo(mkeyLo) < 0)
    						mkeyLo = mkeys;	
    					if(mkeys.compareTo(mkeyHi) > 0)
      				...
      				ConcurrentLinkedQueue<DBKey> q = RelatrixKVJson.findHeadMapKVStream(alias, templateo.getMap())
      						.map(e -> ((Map.Entry<Comparable,DBKey>) e).getValue())
    			    	.collect(Collectors.toCollection(ConcurrentLinkedQueue::new));
      					mkey.addAll(q); // single-threaded merge
    					// compute lo/hi
    		...
    		if(template.getRange() != null) {
    			...
    			if(templateo.getRange() != null) {
    				/*RelatrixKVJson.findHeadMapKVStream(alias,templateo.getRange()).forEach(e -> {
    					DBKey rkeys = ((Map.Entry<Comparable,DBKey>)e).getValue();
    					if(rkeys.compareTo(rkeyLo) < 0)
    						rkeyLo = rkeys;	
    					if(rkeys.compareTo(rkeyHi) > 0)
     				...
     				ConcurrentLinkedQueue<DBKey> q = RelatrixKVJson.findHeadMapKVStream(alias, templateo.getRange())
      						.map(e -> ((Map.Entry<Comparable,DBKey>) e).getValue())
    			    	.collect(Collectors.toCollection(ConcurrentLinkedQueue::new));
      					rkey.addAll(q); // single-threaded merge
    					// compute lo/hi
    	...
    	FindsetUtilJson.getMorphismRange(alias, dkeyLo, mkeyLo, rkeyLo, dkeyHi, mkeyHi, rkeyHi, dkey, mkey, rkey, resultSet);

    	if(DEBUG) {
    		System.out.println(">>Result set size:"+resultSet.size());
    		resultSet.values().iterator().forEachRemaining(e->{
    			try {
    				System.out.println(">>"+RelatrixKVJson.get(alias,e));
    			} catch (IllegalAccessException | IOException e1) {}
    		});
    	}
		...
		if( buffer == null || needsIter) {
			...
			if( iter.hasNext()) {
	    		try {
	    			DBKey dbkey = (DBKey) iter.next();
	    			if(alias == null) {
	    				nextit = (AbstractRelation) RelatrixKVJson.get(dbkey); // primary DBKey for AbstractRelation
	    			} else {
