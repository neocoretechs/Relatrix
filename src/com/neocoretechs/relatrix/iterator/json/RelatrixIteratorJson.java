	...
	protected Result iterateDmr() throws IllegalAccessException, IOException, ClassNotFoundException {
	    ...
	    if( identity ) {
	    	...
	    	for(int i = 0; i < tuples.length(); i++) {
	    		if( DEBUG ) {
	    			System.out.println("RelatrixIteratorJson.iterateDmr() before iteration of "+i+" tuple:"+tuples.get(i));
	    		}
	    		tuples.set(i, buffer.iterate_dmr(dmr_return));
	    		if( DEBUG ) {
	    			System.out.println("RelatrixIteratorJson.iterateDmr() after iteration of "+i+" tuple:"+tuples.get(i));
	    		}
	    	}
	    }
		return tuples;
