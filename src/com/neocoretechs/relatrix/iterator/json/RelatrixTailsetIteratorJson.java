 ...
 *
 */
public class RelatrixTailsetIteratorJson implements Iterator<Result> {
    ...
    public RelatrixTailsetIteratorJson() {}
    /**
    	...
    	try {
    		...
    		if(template.getDomain() != null) {
    			...
    			if(templateo.getDomain() != null) {
    				/*RelatrixKVJson.findTailMapKVStream(templateo.getDomain()).forEach(e -> {
     				...
     				ConcurrentLinkedQueue<DBKey> q = RelatrixKVJson.findTailMapKVStream(templateo.getDomain())
    		...
    		if(template.getMap() != null) {
    			...
    			if(templateo.getMap() != null) {
    				/*RelatrixKVJson.findTailMapKVStream(templateo.getMap()).forEach(e -> {
    				...
    				ConcurrentLinkedQueue<DBKey> q = RelatrixKVJson.findTailMapKVStream(templateo.getMap())
    						.map(e -> ((Map.Entry<Comparable,DBKey>) e).getValue())
    						.collect(Collectors.toCollection(ConcurrentLinkedQueue::new));
    		...
    		if(template.getRange() != null) {
    			...
    			if(templateo.getRange() != null) {
    				/*RelatrixKVJson.findTailMapKVStream(templateo.getRange()).forEach(e -> {
      				...
      				ConcurrentLinkedQueue<DBKey> q = RelatrixKVJson.findTailMapKVStream(templateo.getRange())
    						.map(e -> ((Map.Entry<Comparable,DBKey>) e).getValue())
    						.collect(Collectors.toCollection(ConcurrentLinkedQueue::new));
    ...
    public RelatrixTailsetIteratorJson(Alias alias, AbstractRelation template, AbstractRelation templateo, short[] dmr_return) throws IOException {
    	...
    	try {
    		...
    		if(template.getDomain() != null) {
    			...
    			if(templateo.getDomain() != null) {
    				/*RelatrixKVJson.findTailMapKVStream(alias,templateo.getDomain()).forEach(e -> {
       				...
       				ConcurrentLinkedQueue<DBKey> q = RelatrixKVJson.findTailMapKVStream(alias, templateo.getDomain())
    						.map(e -> ((Map.Entry<Comparable,DBKey>) e).getValue())
    						.collect(Collectors.toCollection(ConcurrentLinkedQueue::new));
    				dkey.addAll(q); // single-threaded merge
    				// compute lo/hi
    		...
    		if(template.getMap() != null) {
    			...
    			if(templateo.getMap() != null) {
    				/*RelatrixKVJson.findTailMapKVStream(alias,templateo.getMap()).forEach(e -> {
    					DBKey mkeys = ((Map.Entry<Comparable,DBKey>)e).getValue();
    					if(mkeys.compareTo(mkeyLo) < 0)
    						mkeyLo = mkeys;	
    					if(mkeys.compareTo(mkeyHi) > 0)
    				...
    				ConcurrentLinkedQueue<DBKey> q = RelatrixKVJson.findTailMapKVStream(alias, templateo.getMap())
    						.map(e -> ((Map.Entry<Comparable,DBKey>) e).getValue())
    						.collect(Collectors.toCollection(ConcurrentLinkedQueue::new));
    				mkey.addAll(q); // single-threaded merge
    				// compute lo/hi
    		...
    		if(template.getRange() != null) {
    			...
    			if(templateo.getRange() != null) {
    				/*RelatrixKVJson.findTailMapKVStream(alias,templateo.getRange()).forEach(e -> {
    					DBKey rkeys = ((Map.Entry<Comparable,DBKey>)e).getValue();
    					if(rkeys.compareTo(rkeyLo) < 0)
    						rkeyLo = rkeys;	
    					if(rkeys.compareTo(rkeyHi) > 0)
       				...
       				ConcurrentLinkedQueue<DBKey> q = RelatrixKVJson.findTailMapKVStream(alias, templateo.getRange())
    						.map(e -> ((Map.Entry<Comparable,DBKey>) e).getValue())
    						.collect(Collectors.toCollection(ConcurrentLinkedQueue::new));
    				rkey.addAll(q); // single-threaded merge
    				// compute lo/hi
	...
	public Result next() {
		...
		if( buffer == null || needsIter) {
			if( DEBUGITERATION ) {
	    			System.out.println("RelatrixTailsetIteratorJson.next() before iteration:"+this.toString());
			}
			if( nextit != null )
				buffer = nextit;
