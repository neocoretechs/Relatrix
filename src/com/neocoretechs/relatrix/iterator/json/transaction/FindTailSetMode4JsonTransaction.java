...
public class FindTailSetMode4JsonTransaction extends FindSetMode4JsonTransaction {
    ...
    public FindTailSetMode4JsonTransaction(TransactionId xid, Object darg, char mop, char rop, Object arg1, Object arg2) { 	
	...
	public Iterator<?> createRelatrixIterator(AbstractRelation tdmr) throws IllegalAccessException, IOException {
		...
		if(tdmr.getMap() == null) {
			if(endarg0 instanceof Class) {
				xdmr.setMap((Comparable) RelatrixJsonTransaction.firstKey(xid, (Class)endarg0));
			} else {
		...
		if(tdmr.getRange() == null) {
			if(endarg1 instanceof Class) {
				xdmr.setRange((Comparable) RelatrixJsonTransaction.firstKey(xid, (Class)endarg1));
			} else {
	...
	public Iterator<?> createRelatrixIterator(Alias alias, AbstractRelation tdmr) throws IllegalAccessException, IOException, NoSuchElementException {
		...
		if(tdmr.getMap() == null) {
			if(endarg0 instanceof Class) {
				xdmr.setMap(alias,(Comparable) RelatrixJsonTransaction.firstKey(alias, xid, (Class)endarg0));
			} else {
		...
		if(tdmr.getRange() == null) {
			if(endarg1 instanceof Class) {
				xdmr.setRange(alias,(Comparable) RelatrixJsonTransaction.firstKey(alias, xid, (Class)endarg1));
			} else {
