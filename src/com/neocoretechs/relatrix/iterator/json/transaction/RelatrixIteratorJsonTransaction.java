 ...
 * The N elements of the query. If its an identity morphism (instance of AbstractRelation) of three keys (as in the *,*,* query)
 ...
 *
 */
public class RelatrixIteratorJsonTransaction extends RelatrixIteratorJson {
	private static boolean DEBUG = false;
	private TransactionId xid = null;
	
	public RelatrixIteratorJsonTransaction() {
		super();
	}
	/**
    ...
    public RelatrixIteratorJsonTransaction(TransactionId xid, AbstractRelation template, short[] dmr_return) throws IOException {
    	...
    	try {
			iter = RelatrixKVJsonTransaction.findTailMapKV(xid, template);
    	...
    	if( DEBUG )
			System.out.println("RelatrixIteratorJsonTransaction Id:"+xid+" "+super.toString());
    }
	/**
	 * Pass the array we use to indicate which values to return and element 0 counter
	 * @param alias
