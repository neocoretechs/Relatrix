 ...
 * which also adds validation. Transactions via RelatrixKVTransaction.
 * @author Jonathan Groff Copyright (C) NeoCoreTechs 2022
 *
 */
public final class IndexInstanceTableTransaction implements IndexInstanceTableInterface {
	public static boolean DEBUG = false;
	LinkedHashSet<Class> classCommits = new LinkedHashSet<Class>();
	DBKey lastKey;
	DBKey lastGoodKey;
	private static Object mutex = new Object();
	public IndexInstanceTableTransaction() {
			synchronized(mutex) {
				...
				try {
					...
					System.out.printf("<<Cannot establish index for object instance storage, must reconcile tables and directories in %s before continuing", RelatrixKVTransaction.getTableSpaceDirectory());
					e.printStackTrace();
					System.exit(1);
				}
	...
	public void put(DBKey index, Comparable instance) throws IllegalAccessException, IOException, ClassNotFoundException {
		synchronized(mutex) {
			...
			try {
				RelatrixKVTransaction.transactionalStore(index, instance);
			...
			try {
				RelatrixKVTransaction.transactionalStore(instance, index);
			} catch(DuplicateKeyException dke) {
				dke.printStackTrace();
	...
	public void delete(DBKey index) throws IllegalAccessException, IOException, DuplicateKeyException, ClassNotFoundException {
		synchronized(mutex) {
			...
			if(index.isValid() ) {
				...
				if(instance != null) {
					RelatrixKVTransaction.remove(instance);
					classCommits.add(instance.getClass());
				}						
			} else {
			...
			RelatrixKVTransaction.remove(index);
			classCommits.add(index.getClass());	
		}
	}
		...
		synchronized(mutex) {
			synchronized(classCommits) {
				...
				while(it.hasNext()) {
					...
					RelatrixKVTransaction.transactionCommit(c);
				}
				classCommits.clear();
				setLastGoodKey();
			}
	...
	public void rollback() throws IOException, IllegalAccessException {
		synchronized(mutex) {
			synchronized(classCommits) {
				Iterator<Class> it = classCommits.iterator();
				while(it.hasNext())
					RelatrixKVTransaction.transactionRollback(it.next());
				classCommits.clear();
			}
	...
	public void checkpoint() throws IllegalAccessException, IOException {
		synchronized(mutex) {
			synchronized(classCommits) {
				Iterator<Class> it = classCommits.iterator();
				while(it.hasNext()) {
					RelatrixKVTransaction.transactionCheckpoint(it.next());
				}
			}
		}	
	}
	...
	public Object getByIndex(DBKey index) throws IllegalAccessException, IOException, ClassNotFoundException {
		//synchronized(mutex) {
			return RelatrixKVTransaction.get(index);
		//}
	}
	/**
	...
	public DBKey getByInstance(Object instance) throws IllegalAccessException, IOException, ClassNotFoundException {
		//synchronized(mutex) {
			return (DBKey) RelatrixKVTransaction.get((Comparable) instance);
		//}
	}

	@Override
	public Object lastKey(Class<DBKey> class1) throws IllegalAccessException, IOException {
		return RelatrixKVTransaction.lastKey(DBKey.class);
	}
