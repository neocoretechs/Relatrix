...
public class TCPKVIteratorTransactionWorker implements Runnable {
    ...
    public TCPKVIteratorTransactionWorker(SocketChannel datasocket, String iteratorClass, ClassLoader classLoader) throws IOException, ClassNotFoundException {
		...
		try {
			while(shouldRun) {
				...
				if( iori.getMethodName().equals("close") ) {
					RelatrixKVTransactionServer.IteratorServerProcesses.removeIterator(iori.getSession(), iori.getIteratorId());
					session = null;
				} else {
					session = iori.getSession();
					// Get the iterator linked to this session
					Object itInst = RelatrixKVTransactionServer.IteratorServerProcesses.getIterator(session, iori.getIteratorId());
					if( itInst == null ) {
			...
			workerHandler.close();
			if(session != null)
				RelatrixKVTransactionServer.IteratorServerProcesses.removeIterator(session);
	...
	public static void main(String args[]) throws Exception {
		...
		SynchronizedThreadManager.getInstance().spin(new TCPKVIteratorTransactionWorker(SocketChannel.open(new InetSocketAddress(args[0],Integer.parseInt(args[1]))),args[2],new HandlerClassLoader())); // master port, class
	}
}
