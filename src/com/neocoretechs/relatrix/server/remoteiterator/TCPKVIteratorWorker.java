		...
		try {
			while(shouldRun) {
				...
				if( iori.getMethodName().equals("close") ) {
					RelatrixKVServer.IteratorServerProcesses.removeIterator(iori.getSession(), iori.getIteratorId());
					session = null;
				} else {
					session = iori.getSession();
					// Get the iterator linked to this session
					Object itInst = RelatrixKVServer.IteratorServerProcesses.getIterator(session, iori.getIteratorId());
					if( itInst == null ) {
			...
			workerHandler.close();
			if(session != null)
				RelatrixKVServer.IteratorServerProcesses.removeIterator(session);
