...
public class TCPIteratorWorkerJson implements Runnable {
		...
		try {
			while(shouldRun) {
				...
				if( iori.getMethodName().equals("close") ) {
					...
					Object itInst = RelatrixServerJson.IteratorServerProcesses.getIterator(session, iori.getIteratorId());
					if( itInst == null ) {
			...
			workerHandler.close();
			if(session != null)
				RelatrixServerJson.IteratorServerProcesses.removeIterator(session);
	...
	public static void main(String args[]) throws Exception {
		...
		SynchronizedThreadManager.getInstance().spin(new TCPIteratorWorkerJson(SocketChannel.open(new InetSocketAddress(args[0],Integer.valueOf(args[1]))),args[2],new HandlerClassLoader()));
	}
}
