 ...
 *
 */
public class TCPKVIteratorWorkerJson implements Runnable {
		...
		try {
			while(shouldRun) {
				...
				if( iori.getMethodName().equals("close") ) {
					RelatrixKVServerJson.IteratorServerProcesses.removeIterator(iori.getSession(), iori.getIteratorId());
					session = null;
				} else {
					session = iori.getSession();
					// Get the iterator linked to this session
					Object itInst = RelatrixKVServerJson.IteratorServerProcesses.getIterator(session, iori.getIteratorId());
					if( itInst == null ) {
			...
			workerHandler.close();
			if(session != null)
				RelatrixKVServerJson.IteratorServerProcesses.removeIterator(session);
	...
	public static void main(String args[]) throws Exception {
		if( args.length != 2 ) {
			System.out.println("Usage: java com.neocoretechs.relatrix.server.remoteiterator.json.TCPKVIteratorWorkerJson [remote master node] [remote master port] [class]");
		}
		SynchronizedThreadManager.getInstance().spin(new TCPKVIteratorWorkerJson(SocketChannel.open(new InetSocketAddress(args[0],Integer.parseInt(args[1]))),args[2],new HandlerClassLoader())); // master port, class
	}
}
