 ...
 *
 */
public class FindHeadSetStreamMode0 extends FindSetStreamMode0 {
	Object[] endarg;
	public FindHeadSetStreamMode0(char dop, char mop, char rop, Object ... endarg) { 	
		super(dop,mop,rop);
		if(endarg.length != 3)
	...
	protected Stream<?> createRelatrixStream(String alias, Morphism tdmr) throws IllegalAccessException, IOException, NoSuchElementException {
		...
		if(tdmr.getRange() == null) {
			if(endarg[2] instanceof Class) {
				tdmr.setRange(alias,(Comparable) RelatrixKV.lastKey(alias,(Class)endarg[2]));
				xdmr.setRangeKey(DBKey.nullDBKey); // full range
				ydmr.setRangeKey(DBKey.fullDBKey);
			} else {
