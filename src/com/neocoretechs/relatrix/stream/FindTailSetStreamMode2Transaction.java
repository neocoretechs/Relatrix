...
*/
public class FindTailSetStreamMode2Transaction extends FindTailSetStreamMode2 {
	String xid;
	Object[] endarg;
    public FindTailSetStreamMode2Transaction(String xid, char dop, Object marg, char rop, Object ... endarg) { 	
    	super(dop, marg, rop, endarg);
    	this.xid = xid;
       	if(endarg.length != 2)
    		throw new RuntimeException("Must supply 2 qualifying arguments for TailSetStream domain and range.");
    }
    /**
	...
	public Stream<?> createRelatrixStream(Morphism tdmr) throws IllegalAccessException, IOException {
		...
		if(tdmr.getRange() == null) {
			if(endarg[1] instanceof Class) {
				tdmr.setRange((Comparable) RelatrixKVTransaction.firstKey(xid,(Class)endarg[1]));
	...
	public Stream<?> createRelatrixStream(String alias, Morphism tdmr) throws IllegalAccessException, IOException, NoSuchElementException {
		...
		if(tdmr.getRange() == null) {
			if(endarg[1] instanceof Class) {
				tdmr.setRange(alias,(Comparable) RelatrixKVTransaction.firstKey(alias,xid,(Class)endarg[1]));
				xdmr.setRangeKey(DBKey.nullDBKey); // full range
				ydmr.setRangeKey(DBKey.fullDBKey);
			} else {
