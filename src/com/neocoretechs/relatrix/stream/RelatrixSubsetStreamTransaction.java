...
public class RelatrixSubsetStreamTransaction<T> extends RelatrixSubsetStream<T> {
    ...
    public RelatrixSubsetStreamTransaction(String xid, Morphism template, Morphism template2, short[] dmr_return) throws IOException {
    	...
    	try {
			//stream = RelatrixKVTransaction.findSubMapStream(xid, template, template2);
      		Spliterator<?> spliterator = Spliterators.spliteratorUnknownSize(new RelatrixSubsetIteratorTransaction(xid, template, template2, dmr_return), RelatrixKV.characteristics);
     ...
     * @throws IOException 
     */
    public RelatrixSubsetStreamTransaction(String alias, String xid, Morphism template, Morphism template2, short[] dmr_return) throws IOException {
    	this.dmr_return = dmr_return;
    	identity = RelatrixStream.isIdentity(this.dmr_return);
    	try {
			//stream = RelatrixKVTransaction.findSubMapStream(xid, template, template2);
      		Spliterator<?> spliterator = Spliterators.spliteratorUnknownSize(new RelatrixSubsetIteratorTransaction(alias, xid, template, template2, dmr_return), RelatrixKV.characteristics);
    		stream = StreamSupport.stream(spliterator, true);
		} catch (IllegalArgumentException e) {
			throw new IOException(e);
		}
