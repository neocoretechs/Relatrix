...
/**
 * The set of tests verifies the lower level {@link KeySet} {@link PrimaryKeySet} functions in the {@link  Relatrix} <p/>
		...
		*/
		...
		public static void battery1(String[] argv) throws Exception {
			...
			for(int i = min; i < max; i++) {
				...
				DomainMapRange identity = Relatrix.store(d, m, r);
		...
		public static void battery1AR4(String[] argv) throws Exception {
			int cnt = 0;
			long tims = System.currentTimeMillis();
			DomainMapRange prev = (DomainMapRange) Relatrix.firstKey(DomainMapRange.class);
		...
		public static void battery1AR4A(String[] argv) throws Exception {
			int cnt = 0;
			long tims = System.currentTimeMillis();
			DBKey prev = (DBKey) RelatrixKV.firstKey(DBKey.class);
		...
		public static void battery1AR44(String[] argv) throws Exception {
			...
			if(its != null) {
				...
				while(its.hasNext()) {
					DomainMapRange nex = (DomainMapRange) its.next();
					pk = (DomainMapRange) IndexResolver.getIndexInstanceTable().getByIndex(nex.getIdentity()); 
		...
		public static void battery1AR5(String[] argv) throws Exception {
			...
			if(its != null) {
				while(its.hasNext()) {
					...
					i = IndexResolver.getIndexInstanceTable().getByIndex((DBKey) nex.getValue()); 
					if( i == null ) {
		...
		public static void battery1AR55(String[] argv) throws Exception {
			...
			Object i;
			long tims = System.currentTimeMillis();
			Iterator<?> its = Relatrix.entrySet( DomainMapRange.class);
		...
		public static void battery1AR12(String[] argv) throws Exception {
			...
			if( c != null ) {
				...
				while(its.hasNext()) {
					...
					DBKey db = IndexResolver.getIndexInstanceTable().getByInstance(nexe.getKey()); // get the DBKey for this instance integer
					DomainMapRange keyset = (DomainMapRange) IndexResolver.getIndexInstanceTable().getByIndex(nexe.getValue());
		...
		public static void battery1AR14(String[] argv) throws Exception {
			...
			if(c != null) {
				...
				while(its.hasNext()) {
					Comparable nex = (Comparable) its.next();
					Map.Entry<DomainMapRange,DBKey> nexe = (Map.Entry<DomainMapRange,DBKey>)nex;
					DBKey db = IndexResolver.getIndexInstanceTable().getByInstance(nexe.getKey()); // get the DBKey for this instance 
