 ...
 * and verifying findHeadSet retrieval for transaction context.
 * NOTES:
 * program arguments are _database
 * @author Jonathan Groff Copyright (C) NeoCoreTechs 2021
 *
 */
public class EmbeddedRetrievalBatteryTransaction2 {
	...
	*/
	...
	public static void battery0(String[] argv, TransactionId xid2) throws Exception {
		...
		for(int i = min; i < max; i++) {
			...
			try {
				dmr = RelatrixTransaction.store(xid, fkey, "Has unit", Long.valueOf(i));
				++recs;
	...
	public static void battery1(String[] argv, TransactionId xid2) throws Exception {
		System.out.println("Iterator Battery1 "+xid);
		String fmap;
		long tims = System.currentTimeMillis();
		int recs = 0;
		...
		for(int j = 0; j < ar3.size(); j++) {
			displayLine = 0;
			System.out.println("9."+j+") FindHeadSet(xid,<obj>,<obj>,<obj>) using domain="+((Result)ar3.get(j)).get(0)+",map="+((Result)ar3.get(j)).get(1)+",range="+((Result)ar3.get(j)).get(2));
			it = RelatrixTransaction.findHeadSet(xid2,((Result)ar3.get(j)).get(0), ((Result)ar3.get(j)).get(1), ((Result)ar3.get(j)).get(2));
			while(it.hasNext()) {
				Object o = it.next();
				Result c = (Result)o;
