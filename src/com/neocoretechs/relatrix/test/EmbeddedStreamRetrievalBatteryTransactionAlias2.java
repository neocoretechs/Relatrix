 ...
 * and verifying findHeadStream retrieval for alias functionality in a transaction context.
 * NOTES:
 ...
 *
 */
public class EmbeddedStreamRetrievalBatteryTransactionAlias2 {
	...
	*/
	...
	public static void battery1(String[] argv, Alias alias12, TransactionId xid2) throws Exception {
		...
		displayLine = 0;
		System.out.println("1.) findHeadStream("+alias12+",xid,*,*,*,String.class, String.class, Long.class)...");
		RelatrixTransaction.findHeadStream(alias12,xid2, "*", "*", "*",String.class, String.class, Long.class).forEach(o->{
			Result c = (Result)o;
			displayCtrl();
		...
		displayLine = 0;
		System.out.println("2.) findHeadStream("+alias12+",xid,*,*,?,String.class, String.class, Long.class)...");		
		RelatrixTransaction.findHeadStream(alias12,xid2, "*", "*", "?",String.class, String.class, Long.class).forEach(o->{
			Result c = (Result)o;
			displayCtrl();
			if(DISPLAY || DISPLAYALL)
		...
		displayLine = 0;
		System.out.println("3.) findHeadStream("+alias12+",xid,*,?,*,String.class, String.class, Long.class)...");		
		RelatrixTransaction.findHeadStream(alias12,xid2, "*", "?", "*",String.class, String.class, Long.class).forEach(o->{
			Result  c = (Result )o;
			displayCtrl();
			if(DISPLAY || DISPLAYALL)
		...
		displayLine = 0;
		System.out.println("4.) findHeadStream("+alias12+",xid,?,*,*.String.class, String.class, Long.class)...");		
		RelatrixTransaction.findHeadStream(alias12,xid2, "?", "*", "*",String.class, String.class, Long.class).forEach(o->{
			Result  c = (Result )o;
			displayCtrl();
			if(DISPLAY || DISPLAYALL)
		...
		displayLine=0;
		System.out.println("5.) findHeadStream("+alias12+",xid,*,?,?,String.class, String.class, Long.class)...");		
		RelatrixTransaction.findHeadStream(alias12,xid2, "*", "?", "?",String.class, String.class, Long.class).forEach(o->{
		...
		displayLine = 0;
		System.out.println("6.) findHeadStream("+alias12+",xid,?,*,?,String.class, String.class, Long.class)...");		
		RelatrixTransaction.findHeadStream(alias12,xid2, "?", "*", "?",String.class, String.class, Long.class).forEach(o->{
			Result2 c = (Result2)o;
			displayCtrl();
			if(DISPLAY || DISPLAYALL)
		...
		displayLine = 0;
		System.out.println("7.) findHeadStream("+alias12+",xid,?,?,*,String.class, String.class, Long.class)...");		
		RelatrixTransaction.findHeadStream(alias12,xid2, "?", "?", "*",String.class, String.class, Long.class).forEach(o->{
			Result2 c = (Result2)o;
			displayCtrl();
			if(DISPLAY || DISPLAYALL)
		...
		displayLine = 0;
		System.out.println("8.) findHeadStream("+alias12+",xid,?,?,?,String.class, String.class, Long.class)...");		
		RelatrixTransaction.findHeadStream(alias12,xid2, "?", "?", "?",String.class, String.class, Long.class).forEach(o->{
			Result3 c = (Result3)o;
			displayCtrl();
			if(DISPLAY || DISPLAYALL)
		...
		for(int j = 0; j < ar3.size(); j++) {
			displayLine = 0;
			System.out.println("8."+j+") findHeadStream("+alias12+",xid,?,?,?,<obj>,<obj>,<obj>) using domain="+((Result)ar3.get(j)).get(0)+",map="+((Result)ar3.get(j)).get(1)+",range="+((Result)ar3.get(j)).get(2));
			RelatrixTransaction.findHeadStream(alias12,xid2,"?","?","?",((Result)ar3.get(j)).get(0), ((Result)ar3.get(j)).get(1), ((Result)ar3.get(j)).get(2)).forEach(o->{
				Result c = (Result)o;
				displayCtrl();
				if(DISPLAY || DISPLAYALL)
					System.out.println(displayLine+"="+c);
			...
			System.out.println("8A."+j+") findHeadStream("+alias12+",xid,?,*,*,<obj>,String.class, String.class) using domain="+((Result)ar3.get(j)).get(0));		
			RelatrixTransaction.findHeadStream(alias12,xid2, "?","*", "*", ((Result)ar3.get(j)).get(0), String.class, String.class).forEach(o->{
				Result c = (Result)o;
				displayCtrl();
				if(DISPLAY || DISPLAYALL)
					System.out.println(displayLine+"="+c);
		...
		for(int j = 0; j < ar3.size(); j++) {
			displayLine = 0;
			System.out.println("9."+j+") findHeadStream("+alias12+",xid2,<obj>,<obj>,<obj>) using domain="+((Result)ar3.get(j)).get(0)+",map="+((Result)ar3.get(j)).get(1)+",range="+((Result)ar3.get(j)).get(2));
			RelatrixTransaction.findHeadStream(alias12,xid2, ((Result)ar3.get(j)).get(0), ((Result)ar3.get(j)).get(1), ((Result)ar3.get(j)).get(2)).forEach(o->{
				Result c = (Result)o;
				displayCtrl();
				if(DISPLAY || DISPLAYALL)
			...
			displayLine=0;
			//RelatrixHeadsetIterator.DEBUG = true;
			System.out.println("10."+j+") findHeadStream("+alias12+",xid,*,*,<obj>,String.class, String.class) using range="+((Result)ar.get(j)).get(0));		
			RelatrixTransaction.findHeadStream(alias12,xid2, "*", "*", ((Result)ar.get(j)).get(0), String.class, String.class).forEach(o->{
				Result c = (Result)o;
				displayCtrl();
				if(DISPLAY || DISPLAYALL)
					System.out.println(displayLine+"="+c);
		...
		for(int j = 0; j < ar.size(); j++) {
			displayLine = 0;
			//RelatrixHeadsetIterator.DEBUG = true;
			System.out.println("11."+j+") findHeadStream("+alias12+",xid,*,<obj>,*, String.class, Long.class) using map="+((Result)am.get(j)).get(0));		
			RelatrixTransaction.findHeadStream(alias12,xid2, "*", ((Result)am.get(j)).get(0), "*",String.class, Long.class).forEach(o->{
				Result c = (Result)o;
				displayCtrl();
				if(DISPLAY || DISPLAYALL)
					System.out.println(displayLine+"="+c);
			...
			displayLine =0;
			System.out.println("12."+j+") findHeadStream("+alias12+",xid,<obj>,*,*,String.class, Long.class) using domain="+((Result)ad.get(j)).get(0));		
			RelatrixTransaction.findHeadStream(alias12,xid2, ((Result)ad.get(j)).get(0), "*", "*",String.class, Long.class).forEach(o->{
				Result c = (Result)o;
				displayCtrl();
				if(DISPLAY || DISPLAYALL)
					System.out.println(displayLine+"="+c);
		...
		for(int j = 0; j < ar2.size(); j++) {
			...
			displayLine = 0;
			System.out.println("13."+j+") findHeadStream("+alias12+",xid,*,<obj>,<obj>,String.class) using map="+((Result)ar2.get(j)).toArray()[0]+" range="+((Result)ar2.get(j)).toArray()[1]);		
			RelatrixTransaction.findHeadStream(alias12,xid2, "*", ((Result)ar2.get(j)).toArray()[0], ((Result)ar2.get(j)).toArray()[1], String.class).forEach(o->{
				Result c = (Result)o;
				displayCtrl();
				if(DISPLAY || DISPLAYALL)
					System.out.println(displayLine+"="+c);
			...
			displayLine = 0;
			System.out.println("14."+j+") findHeadStream("+alias12+",xid,<obj>,*,<obj>,String.class) using domain="+((Result)ar2dr.get(j)).toArray()[0]+", range="+((Result)ar2dr.get(j)).toArray()[1]);		
			RelatrixTransaction.findHeadStream(alias12,xid2, ((Result)ar2dr.get(j)).toArray()[0], "*", ((Result)ar2dr.get(j)).toArray()[1], String.class).forEach(o->{
				Result c = (Result)o;
				displayCtrl();
				if(DISPLAY || DISPLAYALL)
					System.out.println(displayLine+"="+c);
		...
		for(int j = 0; j < ar2.size(); j++) {
			displayLine=0;
			System.out.println("15."+j+") findHeadStream("+alias12+",xid,<obj>,<obj>,*, Long.class) using domain="+((Result)ar2dm.get(j)).toArray()[0]+", map="+((Result)ar2dm.get(j)).toArray()[1]);		
			RelatrixTransaction.findHeadStream(alias12,xid2, ((Result)ar2dm.get(j)).toArray()[0], ((Result)ar2dm.get(j)).toArray()[1], "*", Long.class).forEach(o->{
				Result c = (Result)o;
				displayCtrl();
				if(DISPLAY || DISPLAYALL)
					System.out.println(displayLine+"="+c);
		...
		for(int j = 0; j < ar.size(); j++) {
			displayLine=0;
			System.out.println("16."+j+") findHeadStream("+alias12+",xid2,?,?,<obj>, String.class, String.class) using range="+((Result)ar.get(j)).get(0));		
			RelatrixTransaction.findHeadStream(alias12,xid2, "?", "?", ((Result)ar.get(j)).get(0), String.class, String.class).forEach(o->{
				Result c = (Result)o;
				displayCtrl();
				if(DISPLAY || DISPLAYALL)
			...
			displayLine=0;
			System.out.println("17."+j+") findHeadStream("+alias12+",xid,?,<obj>,?, String.class, Long.class) using map="+((Result)am.get(j)).get(0));		
			RelatrixTransaction.findHeadStream(alias12,xid2, "?", ((Result)am.get(j)).get(0), "?", String.class, Long.class).forEach(o->{
				Result2 c = (Result2)o;
				displayCtrl();
				if(DISPLAY || DISPLAYALL)
			...
			displayLine=0;
			System.out.println("18."+j+") findHeadStream("+alias12+",xid,<obj>,?,?, String.class, Long.class) using domain="+((Result)ad.get(j)).get(0));		
			RelatrixTransaction.findHeadStream(alias12,xid2, ((Result)ad.get(j)).get(0), "?", "?", String.class, Long.class).forEach(o->{
				Result c = (Result)o;
				displayCtrl();
				if(DISPLAY || DISPLAYALL)
		...
		for(int j = 0; j < ar2.size(); j++) {
			displayLine=0;
			System.out.println("19."+j+") findHeadStream("+alias12+",xid,?,<obj>,<obj>, String.class) using map="+((Result)ar2.get(j)).get(0)+" range="+((Result)ar2.get(j)).get(1));		
			RelatrixTransaction.findHeadStream(alias12,xid2, "?", ((Result)ar2.get(j)).get(0), ((Result)ar2.get(j)).get(1), String.class).forEach(o->{
				Result c = (Result)o;
				displayCtrl();
				if(DISPLAY || DISPLAYALL)
					System.out.println(displayLine+"="+c);
			...
			displayLine =0;
			System.out.println("20."+j+") findHeadStream("+alias12+",xid,<obj>,?,<obj>,String.class) using domain="+((Result)ar2dr.get(j)).get(0)+" range="+ ((Result)ar2dr.get(j)).get(1));		
			RelatrixTransaction.findHeadStream(alias12,xid2, ((Result)ar2dr.get(j)).get(0), "?", ((Result)ar2dr.get(j)).get(1), String.class).forEach(o->{
				Result c = (Result)o;
				displayCtrl();
				if(DISPLAY || DISPLAYALL)
					System.out.println(displayLine+"="+c);
			...
			displayLine =0;
			System.out.println("21."+j+") findHeadStream("+alias12+",xid,<obj>,<obj>,?,Long.class) using domain="+((Result)ar2dm.get(j)).get(0)+" map="+((Result)ar2dm.get(j)).get(1));		
			RelatrixTransaction.findHeadStream(alias12,xid2, ((Result)ar2dm.get(j)).get(0), ((Result)ar2dm.get(j)).get(1), "?",Long.class).forEach(o->{
				Result c = (Result)o;
				displayCtrl();
				if(DISPLAY || DISPLAYALL)
					System.out.println(displayLine+"="+c);
